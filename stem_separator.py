...
This is a backward-compatible wrapper script.
For the full package, use: python -m stem_separator
...
# Maintain backward compatibility by delegating to the new package
if __name__ == "__main__":
    try:
        from stem_separator.cli import main
        main()
    except ImportError:
        # Fallback for when package is not installed but script is run directly
        # This ensures the original functionality still works
        print("Note: For full features, install the package: pip install -e .")
        print("Running in compatibility mode...\n")

        # Import and run the legacy code path
        import os
        import argparse
        import subprocess
        import shutil
        ...
        FORMATS = {
            'wav': {'ext': '.wav', 'description': 'Lossless WAV (default)', 'requires_conversion': False},
            'mp3': {'ext': '.mp3', 'codec': 'libmp3lame', 'bitrate': '320k', 'description': 'MP3 320kbps'},
            'flac': {'ext': '.flac', 'codec': 'flac', 'description': 'Lossless FLAC'},
            'ogg': {'ext': '.ogg', 'codec': 'libvorbis', 'quality': '10', 'description': 'Ogg Vorbis'},
            'aac': {'ext': '.aac', 'codec': 'aac', 'bitrate': '256k', 'description': 'AAC 256kbps'}
        }

        PRESETS = {
            'all': {'description': 'All stems (default)', 'include_all': True},
            'karaoke': {'description': 'Everything except vocals', 'exclude': ['vocals']},
            'acapella': {'description': 'Vocals only', 'include': ['vocals']},
            'instrumental': {'description': 'Everything except vocals', 'exclude': ['vocals']}
        }
        ...
        def parse_stem_selection(stems_arg, model_sources):
            ...
            if stems_lower in PRESETS:
                ...
                if not result:
                    print(f"Error: Preset '{stems_lower}' results in no stems")
                    sys.exit(1)
                return result
            ...
            if invalid_stems:
                print(f"Error: Invalid stem(s): {', '.join(invalid_stems)}")
        ...
        def download_youtube(url, output_dir, browser=None):
            ...
            cmd = ['yt-dlp', '-x', '--audio-quality', '0', '-o', temp_file, '--no-playlist', '--progress', '--newline']
            if browser:
        ...
        def legacy_main():
            ...
            parser.add_argument('--stems', default=None,
                               help='Stems to export: comma-separated or preset')
            parser.add_argument('--cpu', action='store_true', help='Force CPU mode')
            parser.add_argument('--browser', choices=['chrome', 'firefox', 'edge', 'safari', 'opera', 'brave'],
                               help='Use cookies from browser for YouTube')
            ...
            try:
                ...
                stems_dir = separate_audio(
                    audio_file, temp_dir, model_name=model_name, output_format=output_format,
                    selected_stems=selected_stems, force_cpu=args.cpu
                )
                format_ext = FORMATS[output_format]['ext']
